...
int main(){
    ...
    if(f != NULL){
        ...
        for(int i = 0; i<totalEventos; i++){
            fscanf(f, "%d", &eventos[i].dataEvento.dia);
			fscanf(f, "%d", &eventos[i].dataEvento.mes);
			fscanf(f, "%d", &eventos[i].dataEvento.ano);
			fscanf(f, "%d", &eventos[i].horarioInicio.hora);
			fscanf(f, "%d", &eventos[i].horarioInicio.minuto);
			fscanf(f, "%d", &eventos[i].horarioFim.hora);
			fscanf(f, "%d", &eventos[i].horarioFim.minuto);
			fscanf(f, " %[^\n]", eventos[i].descricao);
			fscanf(f, " %[^\n]", eventos[i].local);
        }
        fclose(f);
...
int insereEvento(struct evento *evento){
    ...
    printf("\nHora de inicio: ");
    scanf("%d", &evento->horarioInicio.hora);
    printf("\nMinuto de inicio: ");
    scanf("%d", &evento->horarioInicio.minuto);
    printf("\n\nHora de fim: ");
    scanf("%d", &evento->horarioFim.hora);
    printf("\nMinuto de fim: ");
    scanf("%d", &evento->horarioFim.minuto);
    printf("\n");
...
void ordenaDataSelectionSort(struct evento *evento, int n){
   ...
   struct evento troca;
    ...
    for( i=0; i<n-1; i++){
        menor = i;
        for( j=i+1; j<n; j++){
            if(evento[i].dataEvento.ano==evento[j].dataEvento.ano){
                if(evento[i].dataEvento.mes==evento[j].dataEvento.mes){
                    if(evento[i].dataEvento.dia==evento[j].dataEvento.dia){
                        if(evento[i].horarioInicio.hora > evento[j].horarioInicio.hora){
                            menor = j;
                        }
                    }else if(evento[j].dataEvento.dia < evento[menor].dataEvento.dia){
                        menor = j;
                    }
                }else if(evento[j].dataEvento.mes < evento[menor].dataEvento.mes){
                    menor = j;
                }
            }else if(evento[j].dataEvento.ano < evento[menor].dataEvento.ano){
                menor = j;
            }
        }
        if(i != menor){
